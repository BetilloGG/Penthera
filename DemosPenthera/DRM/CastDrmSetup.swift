...
class CastDrmSetup: NSObject {
	...
	private static var drmConfigDelegate: CastDrmConfigDelegate!
    ...
    init?(certificateUrl: String?,
        ...
        CastDrmSetup.drmConfigDelegate = CastDrmConfigDelegate(self.userID, sessionID: self.sessionID, customerName: self.customerName)
    }
    ...
    func configure() ->Bool {
        ...
        func createLicenseConfigurationDelegate() ->VirtuosoLicenseManagerDelegate? {
			...
			guard let licenseConfiguration = VirtuosoLicenseConfiguration(suffix: nil, renewal: nil, parameters: nil, headers: nil) else {
				print("Failed to initialized a VirtuosoLicenseConfiguration.")
				return nil
			}

            // One of the following calls is required for this code to work.
			delegate.addLicense(licenseConfiguration)
			// delegate.addLicense(<#T##config: VirtuosoLicenseConfiguration##VirtuosoLicenseConfiguration#>, forAssetID: <#T##String#>)
		...
		CastLabsAVAssetResourceLoaderDelegate.setDrmConfig(CastDrmSetup.drmConfigDelegate)
        return true
    }
